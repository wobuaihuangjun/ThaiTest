...
public class ThaiRevMinMatchSegmenter extends AbstractThaiSegmenter {
    private Matcher revMinMatcher;
    private POS pos;

    public ThaiRevMinMatchSegmenter() {
        this.revMinMatcher = new ReverseMinimumMatcher(ThaiCoreDictionary.thaiDictionary.dictionaryTrie);
        this.pos = new ThaiPOS();
    }

    protected List<Term> segment(String[] sentence) {
        List<Term> ret = this.revMinMatcher.segment(sentence);
